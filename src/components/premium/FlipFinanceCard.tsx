...
export default function FlipFinanceCard({
  ...
  return (
    <button
      ...
      className="group relative h-[244px] w-full [perspective:1200px] text-left"
      aria-label={`${locationName} gider kartı detayı`}
    >
      <div
        className={cn(
          'relative h-full w-full rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition-transform duration-500 [transform-style:preserve-3d]',
      ...
      >
        ...
        <div className="absolute inset-0 rounded-3xl bg-slate-900 p-5 text-white [transform:rotateY(180deg)] [backface-visibility:hidden]">
          <div className="flex items-start justify-between">
